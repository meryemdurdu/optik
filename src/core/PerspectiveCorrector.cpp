...
WarpResult PerspectiveCorrector::findAndWarp(const cv::Mat& bgr, bool wantDebug) const {
    logging::Logger::ScopedTimer timer("PERSPECTIVE", "PerspectiveCorrector::findAndWarp");
    WarpResult R;
    if (bgr.empty()) {
        LOG_WARN("CAMERA", "Frame bos geldi, warping atlandi");
        return R;
    }

    CornerResult C = finder_.processFrame(bgr, wantDebug);
    if (wantDebug) {
        R.debug = C.debug_bgr.empty() ? bgr.clone() : C.debug_bgr;
    }

    if (!C.paper_ok) {
        LOG_WARN("PERSPECTIVE", "Optik form tespit edilemedi");
        R.ok = false;
        return R;
    }
    ...
    if (R.ok) {
        LOG_INFO("PERSPECTIVE", "Form warping basarili");
    } else {
        LOG_ERROR("PERSPECTIVE", "Warped cikti olusturulamadi");
    }
