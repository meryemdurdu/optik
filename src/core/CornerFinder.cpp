...
CornerFinder::CornerFinder(int outW, int outH) : outW_(outW), outH_(outH) {}

std::vector<Point2f> CornerFinder::orderTLTRBRBL(const std::vector<Point2f>& pts,
                                                 Point2f C) const {
    if (pts.size() != 4) {
        return pts;
    }
...
bool CornerFinder::findCornerSquares(const Mat& gray,
    ...
    for (auto& c : contours) {
        ...
        if (area < 50 || area > 15000)
            continue;

        Rect br = boundingRect(c);
        float ar = static_cast<float>(br.width) / static_cast<float>(br.height);
        if (ar < 0.7f || ar > 1.4f)
            continue;
    ...
    if (dbg)
        drawContours(*dbg, candidateContours, -1, Scalar(0, 255, 255), 2);

    if (candidateContours.size() < 4) {
        LOG_DEBUG("CORNER", "Yeterli marker bulunamadi: " +
                                 std::to_string(candidateContours.size()) + "/4");
        if (dbg) {
            std::string msg = "Yeterli aday yok: " +
                               std::to_string(candidateContours.size()) + "/4";
    ...
    std::vector<std::vector<Point>> finalFour(candidateContours.begin(),
                                              candidateContours.begin() +
                                                  std::min(static_cast<size_t>(4), candidateContours.size()));

    if (dbg)
        drawContours(*dbg, finalFour, -1, Scalar(0, 255, 0), 3);

    std::vector<Point2f> centers;
    centers.reserve(finalFour.size());
    ...
    if (centers.size() != 4) {
        LOG_WARN("CORNER", "Marker merkezleri 4'e esit degil");
        if (dbg)
            putText(*dbg, "Merkez hesaplama hatasi", {20, 70}, FONT_HERSHEY_SIMPLEX, 0.8,
                    {0, 0, 255}, 2);
        return false;
    }
    ...
    if (dbg) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            putText(*dbg, labels[i], corners[i] + Point2f(15, 0), FONT_HERSHEY_SIMPLEX, 0.8,
                    colors[i], 2);
        }
...
CornerResult CornerFinder::processFrame(const Mat& bgr, bool debug_on) const {
    logging::Logger::ScopedTimer timer("PERSPECTIVE", "CornerFinder::processFrame");

    CornerResult R;
    if (bgr.empty()) {
        LOG_WARN("CORNER", "Bos frame ile kose aramasi yapilamaz");
        return R;
    }

    LOG_DEBUG("CORNER", "Frame boyutu: " + std::to_string(bgr.cols) + "x" +
    ...
    if (!R.paper_ok) {
        LOG_WARN("CORNER", "Kose tespiti basarisiz");
        return R;
    }

    std::vector<Point2f> dstPoints = {
        {0.f, 0.f},
        {static_cast<float>(outW_ - 1), 0.f},
        {static_cast<float>(outW_ - 1), static_cast<float>(outH_ - 1)},
        {0.f, static_cast<float>(outH_ - 1)}};
    ...
    for (int i = 0; i < 4; ++i)
        R.markers_orig[i] = srcPoints[i];

    LOG_INFO("CORNER", "Kose tespiti ve warping tamamlandi");
    return R;
}
